...
public class Mano {
	...
	public int valor_jugada() {
		...
		switch (_mejorJugada) {
		case "Straight Flush": {
			valor= 2500000+_max_escalera;
			break;
		}
		case "Four of a Kind": {
			valor= 2400000+_val_aux1+_val_aux2;	
			break;
		}
		case "Full House": {
			valor= 2300000+_val_aux1*100+_val_aux2;	
			break;
		}
		case "Flush": {
			valor= 2200000;
		...
		case "Straight": {
			valor= 2100000+_max_escalera;
			break;
		}
		case "Three of a Kind": {	
			valor= 2000000+ _val_aux1*10000+valor_residual(2, _val_aux1, 0);
			break;
		}
		case "Two Pair": {
			valor=1900000+ _val_aux1*10000+_val_aux2*1000+valor_residual(1, _val_aux1, _val_aux2);
			break;
		}
		case "Pair": {
			valor=1800000+ _val_aux1*10000+valor_residual(3, _val_aux1, 0);
			break;
		}
		case "High Card": {
			valor=valor_residual(5, 0, 0);
			break;
		}
		default:
			throw new IllegalArgumentException("Unexpected value: " + _mejorJugada);
		}
		return valor;
	}
	
	private int valor_residual(int num_cartas,int comp1,int comp2) {
		int escala= (int) Math.pow(10, num_cartas);
		int valor=0;
		int i = _mano.size()-1;
		int cont=0;
		while (i >=0 &&cont<num_cartas) {
		    int num_carta = _mano.get(i).getValorNumerico();
		    if (num_carta != comp1 && num_carta != comp2) {
		        valor += escala * num_carta;
		        cont++;
		        escala/=10;
		    }
		    i--;
